...
- Compatibility milestone runs can now append categorized JSONL result logs with stable milestone IDs/names, final PC/drive/media/tape/screen state plus bounded decoded screen text, write compact aggregate JSON summaries with run configuration metadata and manifest content fingerprints, optionally fail acceptance runs on unclassified failures, and resume by skipping milestones that already passed in a previous log
