import XCTest
@testable import C64Core

final class DiskDriveTests: XCTestCase {

    /// Build a minimal 35-track D64 image with a known disk name, ID, and one PRG file entry.
    func makeMinimalD64(diskName: String = "TEST DISK", diskID: String = "AB", fileName: String = "HELLO", fileBlocks: UInt16 = 5) -> Data {
        let totalBytes = 174848
        var image = [UInt8](repeating: 0, count: totalBytes)

        // --- BAM (track 18, sector 0) ---
        let bamOffset = DiskDrive.trackOffset[18]

        // First directory sector pointer
        image[bamOffset + 0] = 18   // directory track
        image[bamOffset + 1] = 1    // directory sector

        // DOS version
        image[bamOffset + 2] = 0x41 // 'A'

        // BAM entries for tracks 1-35 (4 bytes each, starting at offset 4)
        for t in 1...35 {
            let off = bamOffset + t * 4
            if t == 18 {
                image[off] = 0  // no free sectors on directory track
            } else {
                image[off] = UInt8(DiskDrive.sectorsPerTrack[t])  // all free
            }
        }

        // Disk name at offset $90 (16 bytes, padded with $A0)
        let nameBytes = Array(diskName.utf8)
        for i in 0..<16 {
            if i < nameBytes.count {
                // Convert ASCII to PETSCII uppercase
                var b = nameBytes[i]
                if b >= 0x61 && b <= 0x7A { b -= 32 }
                image[bamOffset + 0x90 + i] = b
            } else {
                image[bamOffset + 0x90 + i] = 0xA0
            }
        }

        // Disk ID at offset $A2 (2 bytes)
        let idBytes = Array(diskID.utf8)
        for i in 0..<2 {
            if i < idBytes.count {
                image[bamOffset + 0xA2 + i] = idBytes[i]
            }
        }

        // DOS type at $A5-$A6
        image[bamOffset + 0xA5] = 0x32  // '2'
        image[bamOffset + 0xA6] = 0x41  // 'A'

        // --- Directory (track 18, sector 1) ---
        let dirOffset = DiskDrive.trackOffset[18] + 1 * 256

        // No next directory sector
        image[dirOffset + 0] = 0
        image[dirOffset + 1] = 0xFF

        // First entry at offset 0 within sector (but entry data starts at byte 2 of 32-byte slot)
        let entryBase = dirOffset + 0  // first 32-byte slot
        image[entryBase + 2] = 0x82   // PRG, closed

        // File first track/sector (we'll put dummy data at track 1, sector 0)
        image[entryBase + 3] = 1   // first track
        image[entryBase + 4] = 0   // first sector

        // Filename at entry+5 (16 bytes padded with $A0)
        let fnBytes = Array(fileName.utf8)
        for i in 0..<16 {
            if i < fnBytes.count {
                var b = fnBytes[i]
                if b >= 0x61 && b <= 0x7A { b -= 32 }
                image[entryBase + 5 + i] = b
            } else {
                image[entryBase + 5 + i] = 0xA0
            }
        }

        // File size in sectors
        image[entryBase + 30] = UInt8(fileBlocks & 0xFF)
        image[entryBase + 31] = UInt8(fileBlocks >> 8)

        return Data(image)
    }

    func testMountAndParseDirectory() {
        let drive = DiskDrive()
        let d64 = makeMinimalD64()
        XCTAssertTrue(drive.mount(d64))
        XCTAssertTrue(drive.isMounted)
        XCTAssertEqual(drive.directory.count, 1, "Should find 1 directory entry")
        XCTAssertEqual(drive.directory.first?.filename, "hello")  // petsciiToChar lowercases PETSCII uppercase
        XCTAssertEqual(drive.directory.first?.typeName, "PRG")
    }

    func testDirectoryParsingStopsOnCyclicSectorChain() {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        let dirOffset = DiskDrive.trackOffset[18] + 1 * 256
        image[dirOffset] = 18
        image[dirOffset + 1] = 1

        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertEqual(drive.directory.count, 1)
        XCTAssertEqual(drive.directory.first?.filename, "hello")
    }

    func testMountExtendedD64SizeWithStandardDirectory() {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0, count: 200704 - image.count))

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.isMounted)
        XCTAssertEqual(drive.directory.first?.filename, "hello")
    }

    func testD64GeometryRecognizesExtendedImagesWithErrorTables() throws {
        let cases: [(size: Int, dataSize: Int, tracks: Int, sectorCount: Int, errorOffset: Int?)] = [
            (174848, 174848, 35, 683, nil),
            (175531, 174848, 35, 683, 174848),
            (196608, 196608, 40, 768, nil),
            (197376, 196608, 40, 768, 196608),
            (200704, 200704, 41, 784, nil),
            (201488, 200704, 41, 784, 200704),
            (205312, 205312, 42, 802, nil),
            (206114, 205312, 42, 802, 205312),
        ]

        for geometryCase in cases {
            let geometry = try XCTUnwrap(
                DiskDrive.d64Geometry(forByteCount: geometryCase.size),
                "Expected \(geometryCase.size)-byte D64 geometry"
            )
            let sectorCount = geometry.sectorsPerTrack[1...geometry.trackCount].reduce(0, +)

            XCTAssertEqual(geometry.dataSize, geometryCase.dataSize)
            XCTAssertEqual(geometry.trackCount, geometryCase.tracks)
            XCTAssertEqual(sectorCount, geometryCase.sectorCount)
            XCTAssertEqual(geometry.errorInfoOffset, geometryCase.errorOffset)
        }
    }

    func testExtendedD64CanReadFileDataFromTrack36() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64(fileBlocks: 1))
        image.append(contentsOf: [UInt8](repeating: 0, count: 196608 - image.count))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))

        let dirOffset = DiskDrive.trackOffset[18] + 1 * 256
        image[dirOffset + 3] = 36
        image[dirOffset + 4] = 0

        let fileOffset = geometry.trackOffsets[36]
        image[fileOffset + 0] = 0
        image[fileOffset + 1] = 4
        image[fileOffset + 2] = 0xA9
        image[fileOffset + 3] = 0x2A

        XCTAssertTrue(drive.mount(Data(image)))
        let entry = try XCTUnwrap(drive.findFile("HELLO"))

        XCTAssertEqual(drive.readFileData(entry), [0xA9, 0x2A])
    }

    func testFileReadStopsOnCyclicSectorChain() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64(fileBlocks: 1))

        let fileOffset = DiskDrive.trackOffset[1]
        image[fileOffset] = 1
        image[fileOffset + 1] = 0
        image[fileOffset + 2] = 0x01
        image[fileOffset + 3] = 0x08
        image[fileOffset + 4] = 0xA9
        image[fileOffset + 5] = 0x2A

        XCTAssertTrue(drive.mount(Data(image)))
        let entry = try XCTUnwrap(drive.findFile("HELLO"))
        let data = drive.readFileData(entry)

        XCTAssertEqual(data.count, 254)
        XCTAssertEqual(Array(data.prefix(4)), [0x01, 0x08, 0xA9, 0x2A])
    }

    func testPlainD64DoesNotExposeSectorErrorInfo() {
        let drive = DiskDrive()

        XCTAssertTrue(drive.mount(makeMinimalD64()))

        XCTAssertFalse(drive.hasSectorErrorInfo)
        XCTAssertNil(drive.readSectorErrorCode(track: 1, sector: 0))
    }

    func testD64WithSectorErrorBytesPreservesErrorCodes() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 683))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        image[errorOffset] = 0x05
        image[errorOffset + 358] = 0x0B // Track 18, sector 1.

        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.hasSectorErrorInfo)
        XCTAssertEqual(drive.readSectorErrorCode(track: 1, sector: 0), 0x05)
        XCTAssertEqual(drive.readSectorErrorCode(track: 18, sector: 1), 0x0B)
        XCTAssertNil(drive.readSectorErrorCode(track: 36, sector: 0))
    }

    func testExtendedD64WithSectorErrorBytesPreservesTrack41ErrorCodes() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0, count: 200704 - image.count))
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 784))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        image[errorOffset + 783] = 0x0F

        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.hasSectorErrorInfo)
        XCTAssertEqual(drive.readSectorErrorCode(track: 41, sector: 15), 0x0F)
        XCTAssertNil(drive.readSectorErrorCode(track: 41, sector: 16))
    }

    func testGenerateDirectoryListing() {
        let drive = DiskDrive()
        let d64 = makeMinimalD64()
        XCTAssertTrue(drive.mount(d64))

        let prg = drive.generateDirectoryListing()

        // Must start with load address $0801
        XCTAssertGreaterThanOrEqual(prg.count, 4, "PRG must have at least load address + end marker")
        XCTAssertEqual(prg[0], 0x01, "Load address lo should be $01")
        XCTAssertEqual(prg[1], 0x08, "Load address hi should be $08")

        // Walk the BASIC program (skip 2-byte load address)
        let base = 0x0801
        var offset = 2  // skip load address
        var lineCount = 0

        while offset + 1 < prg.count {
            let nextPtrLo = Int(prg[offset])
            let nextPtrHi = Int(prg[offset + 1])
            let nextPtr = nextPtrLo | (nextPtrHi << 8)

            if nextPtr == 0 {
                // End of program
                break
            }

            // Line number
            let lineNumLo = Int(prg[offset + 2])
            let lineNumHi = Int(prg[offset + 3])
            let lineNum = lineNumLo | (lineNumHi << 8)

            // Find end of line (0x00 terminator)
            let textStart = offset + 4
            var textEnd = textStart
            while textEnd < prg.count && prg[textEnd] != 0 {
                textEnd += 1
            }

            let textBytes = Array(prg[textStart..<textEnd])
            let text = textBytes.map { b -> Character in
                if b >= 0x20 && b <= 0x7E { return Character(UnicodeScalar(b)) }
                if b == 0x12 { return Character("®") }  // reverse on placeholder
                return Character("?")
            }
            let textStr = String(text)

            print("Line \(lineCount): num=\(lineNum) nextPtr=$\(String(nextPtr, radix: 16)) text=[\(textStr)]")

            // Verify next pointer = base + (textEnd + 1 - 2) = points to byte after terminator
            let expectedNextPtr = base + (textEnd + 1 - 2)
            XCTAssertEqual(nextPtr, expectedNextPtr,
                "Next line pointer should point to byte after 0x00 terminator (line \(lineCount))")

            lineCount += 1

            // Advance to next line
            let nextLineOffset = nextPtr - base + 2  // +2 for load address prefix
            XCTAssertGreaterThan(nextLineOffset, offset, "Must advance forward")
            offset = nextLineOffset
        }

        XCTAssertGreaterThanOrEqual(lineCount, 2, "Should have at least header + footer lines")
        print("Total lines: \(lineCount), total bytes: \(prg.count)")

        // Verify the program ends with 0x00 0x00
        XCTAssertEqual(prg[offset], 0x00, "End marker lo")
        XCTAssertEqual(prg[offset + 1], 0x00, "End marker hi")

        // Simulate what handleLoad does
        let payload = Array(prg[2...])
        let endAddr = 0x0801 + payload.count
        print("Payload size: \(payload.count), endAddr: $\(String(endAddr, radix: 16))")
        print("First 20 payload bytes: \(payload.prefix(20).map { String(format: "%02X", $0) }.joined(separator: " "))")

        // Verify first 2 bytes at $0801 (next-line pointer) are non-zero
        XCTAssertFalse(payload[0] == 0 && payload[1] == 0,
            "First line's next-line pointer at $0801 must NOT be $0000")
    }

    func testEmptyDirectoryListing() {
        // D64 with no files
        let drive = DiskDrive()
        let d64 = makeMinimalD64(fileName: "")
        // Override: clear the file entry
        var image = [UInt8](d64)
        let dirOffset = DiskDrive.trackOffset[18] + 1 * 256
        image[dirOffset + 2] = 0  // clear file type = empty entry
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertEqual(drive.directory.count, 0)

        let prg = drive.generateDirectoryListing()
        XCTAssertGreaterThan(prg.count, 4)

        // Should still have header + footer
        let payload = Array(prg[2...])
        XCTAssertFalse(payload[0] == 0 && payload[1] == 0,
            "Even empty directory should have non-zero first pointer")
    }

    func testDirectoryListingHonorsNameAndTypeFilters() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeSameNamePRGAndSEQD64()))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "$:HELLO,S"))
        let seqListing = readChannelString(drive, channel: 2)
        XCTAssertTrue(seqListing.contains("HELLO"))
        XCTAssertTrue(seqListing.contains("SEQ"))
        XCTAssertFalse(seqListing.contains("PRG"))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "$0:HELLO,P"))
        let prgListing = readChannelString(drive, channel: 2)
        XCTAssertTrue(prgListing.contains("HELLO"))
        XCTAssertTrue(prgListing.contains("PRG"))
        XCTAssertFalse(prgListing.contains("SEQ"))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "$:MISSING"))
        let missingListing = readChannelString(drive, channel: 2)
        XCTAssertFalse(missingListing.contains("HELLO"))
        XCTAssertTrue(missingListing.contains("BLOCKS FREE."))
    }

    func testDirectoryListingAcceptsEqualsTypeFilters() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeSameNamePRGAndSEQD64()))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "$=S"))
        let seqOnly = readChannelString(drive, channel: 2)
        XCTAssertTrue(seqOnly.contains("HELLO"))
        XCTAssertTrue(seqOnly.contains("SEQ"))
        XCTAssertFalse(seqOnly.contains("PRG"))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "$:*=P"))
        let prgOnly = readChannelString(drive, channel: 2)
        XCTAssertTrue(prgOnly.contains("HELLO"))
        XCTAssertTrue(prgOnly.contains("PRG"))
        XCTAssertFalse(prgOnly.contains("SEQ"))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "$0:HELLO=SEQ"))
        let namedSeq = readChannelString(drive, channel: 2)
        XCTAssertTrue(namedSeq.contains("HELLO"))
        XCTAssertTrue(namedSeq.contains("SEQ"))
        XCTAssertFalse(namedSeq.contains("PRG"))
    }

    func testDirectoryListingLoadReportsD64DirectorySectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 683))
        image[174848 + 358] = 23

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertFalse(drive.openFile(channel: 2, filename: "$"))

        XCTAssertEqual(drive.currentCommandStatus, "23, READ ERROR,18,01\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
        XCTAssertFalse(drive.generateDirectoryListing().isEmpty)
    }

    func testFindFileAcceptsDrivePrefixAndFileOptions() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeMinimalD64()))

        XCTAssertNotNil(drive.findFile("0:HELLO"))
        XCTAssertNotNil(drive.findFile(":HELLO,P,R"))
        XCTAssertNotNil(drive.findFile("HEL*,P"))
        XCTAssertTrue(drive.isDirectoryListingRequest("$0"))
        XCTAssertTrue(drive.isDirectoryListingRequest("0:$"))
    }

    func testFindFileHonorsRequestedFileTypeForReadLookup() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeSameNamePRGAndSEQD64()))

        XCTAssertEqual(try XCTUnwrap(drive.findFile("HELLO")).typeName, "PRG")
        XCTAssertEqual(try XCTUnwrap(drive.findFile("HELLO,S")).typeName, "SEQ")
        XCTAssertEqual(try XCTUnwrap(drive.findFile("*,S")).typeName, "SEQ")
        XCTAssertNil(drive.findFile("HELLO,U"))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "HELLO,S,R"))
        XCTAssertEqual(readChannelBytes(drive, channel: 2, count: 3), [0x10, 0x20, 0x30])
    }

    func testFindFileAcceptsDelTypeFilter() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        let dirOffset = DiskDrive.trackOffset[18] + 1 * 256
        image[dirOffset + 2] = 0x80

        XCTAssertTrue(drive.mount(Data(image)))

        let entry = try XCTUnwrap(drive.findFile("HELLO,D"))
        XCTAssertEqual(entry.typeName, "DEL")
        XCTAssertNotNil(drive.findFile("HELLO,DEL"))
        XCTAssertNil(drive.findFile("HELLO,P"))
    }

    func testOpenMissingFileReportsFileNotFoundStatus() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeMinimalD64()))

        XCTAssertFalse(drive.openFile(channel: 2, filename: "MISSING"))

        XCTAssertEqual(drive.currentCommandStatus, "62, FILE NOT FOUND,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testOpenFileWithoutMountedDiskReportsDriveNotReadyStatus() {
        let drive = DiskDrive()

        XCTAssertFalse(drive.openFile(channel: 2, filename: "HELLO"))

        XCTAssertEqual(drive.currentCommandStatus, "74, DRIVE NOT READY,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testMountClearsOpenChannelsAndCommandDiagnosticsButKeepsDriveMemory() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xDE
        image[sectorOffset + 1] = 0xAD

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-W:$0400,1,$7F"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0400,1"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,1,2"))
        XCTAssertEqual(drive.lastMemoryExecuteAddress, 0x0500)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xDE)
        XCTAssertFalse(drive.readByte(channel: 15).eof)

        XCTAssertTrue(drive.mount(makeMinimalD64(diskName: "NEW DISK")))

        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")
        XCTAssertNil(drive.lastMemoryExecuteAddress)
        XCTAssertTrue(drive.readByte(channel: 2).eof)
        XCTAssertTrue(drive.readByte(channel: 15).eof)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0400,1"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 1), [0x7F])
        XCTAssertEqual(drive.diskName, "new disk")
    }

    func testSavePRGWritesDirectoryBAMAndReadableSectorChain() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertEqual(drive.mountedFormat, .d64)
        XCTAssertFalse(drive.hasUnsavedChanges)
        let initialFreeBlocks = drive.freeBlocks()
        let payload = [0x01, 0x08] + Array((0..<300).map { UInt8($0 & 0xFF) })

        XCTAssertTrue(drive.savePRG(filename: "NEWFILE", data: payload))

        let entry = try XCTUnwrap(drive.findFile("NEWFILE"))
        XCTAssertEqual(entry.filename, "newfile")
        XCTAssertEqual(entry.typeName, "PRG")
        XCTAssertEqual(entry.fileSize, 2)
        XCTAssertEqual(drive.readFileData(entry), payload)
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 2)
        XCTAssertTrue(drive.hasUnsavedChanges)
        drive.markChangesSaved()
        XCTAssertFalse(drive.hasUnsavedChanges)

        let firstSector = drive.readSector(track: Int(entry.firstTrack), sector: Int(entry.firstSector))
        XCTAssertEqual(firstSector?[0], 1)
        XCTAssertEqual(firstSector?[1], 1)
    }

    func testSavePRGClearsD64SectorErrorBytesForWrittenSectors() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848] = 23
        image[174848 + 357] = 0x05
        image[174848 + 358] = 0x05

        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.savePRG(filename: "REPAIRED", data: [0x01, 0x08, 0xA9, 0x2A]))

        let entry = try XCTUnwrap(drive.findFile("REPAIRED"))
        XCTAssertEqual(drive.loadFileData(entry), [0x01, 0x08, 0xA9, 0x2A])
        XCTAssertEqual(drive.readSectorErrorCode(track: 1, sector: 0), 0x01)
        XCTAssertEqual(drive.readSectorErrorCode(track: 18, sector: 0), 0x01)
        XCTAssertEqual(drive.readSectorErrorCode(track: 18, sector: 1), 0x01)
    }

    func testSavePRGExtendsFullDirectorySectorOnTrack18() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        for index in 0..<8 {
            XCTAssertTrue(drive.savePRG(filename: "FILE\(index)", data: [0x01, 0x08, UInt8(index), 0x60]))
        }
        let freeBlocksAfterEightFiles = drive.freeBlocks()

        XCTAssertTrue(drive.savePRG(filename: "NINTH", data: [0x01, 0x08, 0xA9, 0x09, 0x60]))

        let firstDirectorySector = try XCTUnwrap(drive.readSector(track: 18, sector: 1))
        let secondDirectorySector = try XCTUnwrap(drive.readSector(track: 18, sector: 2))
        let bamSector = try XCTUnwrap(drive.readSector(track: 18, sector: 0))
        XCTAssertEqual(firstDirectorySector[0], 18)
        XCTAssertEqual(firstDirectorySector[1], 2)
        XCTAssertEqual(secondDirectorySector[0], 0)
        XCTAssertEqual(secondDirectorySector[1], 0xFF)
        XCTAssertNotNil(drive.findFile("NINTH"))
        XCTAssertEqual(drive.directory.count, 9)
        XCTAssertEqual(drive.freeBlocks(), freeBlocksAfterEightFiles - 1)
        XCTAssertEqual(bamSector[18 * 4 + 1] & 0x04, 0x00)
    }

    func testSavePRGDoesNotExtendFullDirectoryWhenNoDataSectorsAvailable() throws {
        let sourceDrive = DiskDrive()
        XCTAssertTrue(sourceDrive.mount(makeBlankWritableD64()))
        for index in 0..<8 {
            XCTAssertTrue(sourceDrive.savePRG(filename: "FILE\(index)", data: [0x01, 0x08, UInt8(index), 0x60]))
        }

        var image = [UInt8](try XCTUnwrap(sourceDrive.exportedD64Image))
        let bamOffset = DiskDrive.trackOffset[18]
        for track in 1...35 where track != 18 {
            let entryOffset = bamOffset + track * 4
            image[entryOffset] = 0
            image[entryOffset + 1] = 0
            image[entryOffset + 2] = 0
            image[entryOffset + 3] = 0
        }

        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertEqual(drive.freeBlocks(), 0)
        let firstDirectoryBefore = try XCTUnwrap(drive.readSector(track: 18, sector: 1))
        let bamBefore = try XCTUnwrap(drive.readSector(track: 18, sector: 0))

        XCTAssertFalse(drive.savePRG(filename: "NINTH", data: [0x01, 0x08, 0xA9, 0x09, 0x60]))

        let firstDirectoryAfter = try XCTUnwrap(drive.readSector(track: 18, sector: 1))
        let bamAfter = try XCTUnwrap(drive.readSector(track: 18, sector: 0))
        XCTAssertEqual(firstDirectoryAfter, firstDirectoryBefore)
        XCTAssertEqual(bamAfter, bamBefore)
        XCTAssertNil(drive.findFile("NINTH"))
        XCTAssertEqual(drive.directory.count, 8)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testSavePRGReportsD64DirectoryReadErrorsWithoutMutating() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 358] = 23

        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertFalse(drive.savePRG(filename: "BLOCKED", data: [0x01, 0x08, 0x60]))

        XCTAssertEqual(drive.currentCommandStatus, "23, READ ERROR,18,01\r")
        XCTAssertNil(drive.findFile("BLOCKED"))
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testSavePRGReportsD64BAMReadErrorsWithoutMutating() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 357] = 23

        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertFalse(drive.savePRG(filename: "BLOCKED", data: [0x01, 0x08, 0x60]))

        XCTAssertEqual(drive.currentCommandStatus, "23, READ ERROR,18,00\r")
        XCTAssertNil(drive.findFile("BLOCKED"))
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testSavedD64ExportsModifiedImageThatCanBeRemounted() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "EXPORT", data: [0x00, 0x20, 0xA9, 0x7F, 0x60]))
        let exported = try XCTUnwrap(drive.exportedD64Image)

        let remounted = DiskDrive()
        XCTAssertTrue(remounted.mount(exported))
        XCTAssertFalse(remounted.hasUnsavedChanges)
        let entry = try XCTUnwrap(remounted.findFile("EXPORT"))
        XCTAssertEqual(remounted.readFileData(entry), [0x00, 0x20, 0xA9, 0x7F, 0x60])
    }

    func testSavePRGRejectsDecodedG64ReadPath() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mountG64(makeMinimalReadableG64()))
        XCTAssertEqual(drive.mountedFormat, .g64)

        XCTAssertFalse(drive.savePRG(filename: "NOPE", data: [0x01, 0x08, 0x60]))

        XCTAssertFalse(drive.hasUnsavedChanges)
        XCTAssertNil(drive.exportedD64Image)
    }

    func testWriteProtectedD64RejectsHighLevelMutations() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "OLD", data: [0x01, 0x08, 0xA9, 0x2A]))
        drive.markChangesSaved()
        drive.setWriteProtected(true)

        XCTAssertFalse(drive.savePRG(filename: "NEW", data: [0x01, 0x08, 0x60]))
        XCTAssertEqual(drive.currentCommandStatus, "26, WRITE PROTECT ON,00,00\r")
        XCTAssertNil(drive.findFile("NEW"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "R:RENAMED=OLD"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "26, WRITE PROTECT ON,00,00\r")
        XCTAssertNotNil(drive.findFile("OLD"))
        XCTAssertNil(drive.findFile("RENAMED"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:OLD"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "26, WRITE PROTECT ON,00,00\r")
        XCTAssertNotNil(drive.findFile("OLD"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=OLD"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "26, WRITE PROTECT ON,00,00\r")
        XCTAssertNil(drive.findFile("COPY"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "N:LOCKED,LP"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "26, WRITE PROTECT ON,00,00\r")
        XCTAssertNotEqual(drive.diskName, "locked")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "26, WRITE PROTECT ON,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "26, WRITE PROTECT ON,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "V"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "26, WRITE PROTECT ON,00,00\r")
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testSavePRGRejectsDuplicateFilenameWithoutChangingFreeBlocks() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "ONCE", data: [0x01, 0x08, 0xA9, 0x2A]))
        let freeAfterFirstSave = drive.freeBlocks()

        XCTAssertFalse(drive.savePRG(filename: "ONCE", data: [0x01, 0x08, 0xEA]))

        XCTAssertEqual(drive.currentCommandStatus, "63, FILE EXISTS,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), freeAfterFirstSave)
        let entry = try XCTUnwrap(drive.findFile("ONCE"))
        XCTAssertEqual(drive.readFileData(entry), [0x01, 0x08, 0xA9, 0x2A])
    }

    func testSavePRGReportsStatusForInvalidOrUnavailableMedia() {
        let drive = DiskDrive()

        XCTAssertFalse(drive.savePRG(filename: "NO DISK", data: [0x01, 0x08, 0x60]))
        XCTAssertEqual(drive.currentCommandStatus, "74, DRIVE NOT READY,00,00\r")

        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertFalse(drive.savePRG(filename: "$", data: [0x01, 0x08, 0x60]))
        XCTAssertEqual(drive.currentCommandStatus, "30, SYNTAX ERROR,00,00\r")

        let g64Drive = DiskDrive()
        XCTAssertTrue(g64Drive.mountG64(makeMinimalReadableG64()))
        XCTAssertFalse(g64Drive.savePRG(filename: "READONLY", data: [0x01, 0x08, 0x60]))
        XCTAssertEqual(g64Drive.currentCommandStatus, "74, DRIVE NOT READY,00,00\r")
    }

    func testSavePRGReplaceSyntaxReusesDirectoryEntryAndFreesOldChain() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()
        XCTAssertTrue(drive.savePRG(filename: "ONCE", data: [0x01, 0x08, 0xA9, 0x2A]))
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 1)

        let replacement = [0x01, 0x08] + Array((0..<300).map { UInt8(($0 &+ 3) & 0xFF) })
        XCTAssertTrue(drive.savePRG(filename: "@0:ONCE,P", data: replacement))

        XCTAssertEqual(drive.directory.count, 1)
        let entry = try XCTUnwrap(drive.findFile("0:ONCE"))
        XCTAssertEqual(entry.fileSize, 2)
        XCTAssertEqual(drive.readFileData(entry), replacement)
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 2)
        XCTAssertTrue(drive.hasUnsavedChanges)
    }

    func testSavePRGReplaceExistingReportsSourceReadErrorWithoutFreeingChain() throws {
        let sourceDrive = DiskDrive()
        XCTAssertTrue(sourceDrive.mount(makeBlankWritableD64WithErrorTable()))
        XCTAssertTrue(sourceDrive.savePRG(filename: "OLD", data: [0x01, 0x08, 0xA9, 0x2A]))
        let oldEntry = try XCTUnwrap(sourceDrive.findFile("OLD"))
        let freeAfterSave = sourceDrive.freeBlocks()

        var image = [UInt8](try XCTUnwrap(sourceDrive.exportedD64Image))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        let ordinal = geometry.trackOffsets[Int(oldEntry.firstTrack)] / 256 + Int(oldEntry.firstSector)
        image[errorOffset + ordinal] = 23

        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertFalse(drive.savePRG(filename: "@0:OLD,P", data: [0x01, 0x08, 0x60]))

        XCTAssertEqual(drive.currentCommandStatus, "23, READ ERROR,01,00\r")
        let entry = try XCTUnwrap(drive.findFile("OLD"))
        XCTAssertEqual(entry.firstTrack, oldEntry.firstTrack)
        XCTAssertEqual(entry.firstSector, oldEntry.firstSector)
        XCTAssertEqual(drive.freeBlocks(), freeAfterSave)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testSavePRGAcceptsDrivePrefixedReplaceSyntax() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "ONCE,P", data: [0x01, 0x08, 0xA9, 0x2A, 0x60]))
        let freeAfterFirstSave = drive.freeBlocks()

        XCTAssertTrue(drive.savePRG(filename: "0:@ONCE,P", data: [0x01, 0x08, 0x60]))

        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")
        XCTAssertEqual(drive.directory.count, 1)
        let entry = try XCTUnwrap(drive.findFile("ONCE,P"))
        XCTAssertEqual(drive.readFileData(entry), [0x01, 0x08, 0x60])
        XCTAssertEqual(drive.freeBlocks(), freeAfterFirstSave)
    }

    func testSavePRGSuccessClearsPreviousCommandStatus() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertFalse(drive.savePRG(filename: "$", data: [0x01, 0x08, 0x60]))
        XCTAssertEqual(drive.currentCommandStatus, "30, SYNTAX ERROR,00,00\r")

        XCTAssertTrue(drive.savePRG(filename: "GOOD", data: [0x01, 0x08, 0x60]))
        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")

        XCTAssertFalse(drive.savePRG(filename: "GOOD", data: [0x01, 0x08, 0x60]))
        XCTAssertEqual(drive.currentCommandStatus, "63, FILE EXISTS,00,00\r")

        XCTAssertTrue(drive.savePRG(filename: "EMPTY", data: []))
        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")

        let empty = try XCTUnwrap(drive.findFile("EMPTY"))
        XCTAssertEqual(empty.fileSize, 0)
        XCTAssertEqual(drive.readFileData(empty), [])
    }

    func testOpenWriteChannelCreatesSEQFileOnClose() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 2, filename: "REPORT,S,W"))
        for byte in Array("HELLO FROM SEQ\r".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)

        let entry = try XCTUnwrap(drive.findFile("REPORT"))
        XCTAssertEqual(entry.typeName, "SEQ")
        XCTAssertEqual(drive.readFileData(entry), Array("HELLO FROM SEQ\r".utf8))
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 1)
        XCTAssertTrue(drive.hasUnsavedChanges)
    }

    func testOpenWriteChannelCreatesZeroBlockSEQFileOnClose() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 2, filename: "EMPTY,S,W"))
        drive.closeChannel(2)

        let entry = try XCTUnwrap(drive.findFile("EMPTY"))
        XCTAssertEqual(entry.typeName, "SEQ")
        XCTAssertEqual(entry.fileSize, 0)
        XCTAssertEqual(entry.firstTrack, 0)
        XCTAssertEqual(entry.firstSector, 0)
        XCTAssertEqual(drive.readFileData(entry), [])
        XCTAssertEqual(drive.loadFileData(entry), [])
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
        XCTAssertTrue(drive.hasUnsavedChanges)
    }

    func testOpenWriteChannelCreatesPRGAndRejectsDuplicateWithoutReplace() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "OUTPUT,P,W"))
        for byte in [0x01, 0x08, 0xA9, 0x2A, 0x60] as [UInt8] {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)

        let entry = try XCTUnwrap(drive.findFile("OUTPUT"))
        XCTAssertEqual(entry.typeName, "PRG")
        XCTAssertEqual(drive.readFileData(entry), [0x01, 0x08, 0xA9, 0x2A, 0x60])

        XCTAssertFalse(drive.openFile(channel: 3, filename: "OUTPUT,P,W"))
        XCTAssertEqual(drive.currentCommandStatus, "63, FILE EXISTS,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 3).eof)
    }

    func testOpenWriteChannelRejectsRELUntilRecordSemanticsAreSupported() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()

        XCTAssertFalse(drive.openFile(channel: 2, filename: "INDEX,L,W"))

        XCTAssertEqual(drive.currentCommandStatus, "30, SYNTAX ERROR,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
        XCTAssertNil(drive.findFile("INDEX,L"))
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testFailedReadOpenClearsPreviousChannelData() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeMinimalD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAA))

        XCTAssertFalse(drive.openFile(channel: 2, filename: "MISSING"))

        XCTAssertEqual(drive.currentCommandStatus, "62, FILE NOT FOUND,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testFailedWriteOpenClearsPreviousChannelData() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "OUTPUT,S,W"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAA))
        drive.closeChannel(2)
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xBB))

        XCTAssertFalse(drive.openFile(channel: 2, filename: "OUTPUT,S,W"))

        XCTAssertEqual(drive.currentCommandStatus, "63, FILE EXISTS,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testInvalidWriteOpenReportsSyntaxErrorAndCanRecover() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertFalse(drive.openFile(channel: 2, filename: "MISSING"))
        XCTAssertEqual(drive.currentCommandStatus, "62, FILE NOT FOUND,00,00\r")

        XCTAssertFalse(drive.openFile(channel: 2, filename: ",S,W"))
        XCTAssertEqual(drive.currentCommandStatus, "30, SYNTAX ERROR,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)

        XCTAssertFalse(drive.openFile(channel: 2, filename: "$,S,W"))
        XCTAssertEqual(drive.currentCommandStatus, "30, SYNTAX ERROR,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)

        XCTAssertTrue(drive.openFile(channel: 2, filename: "RECOVER,S,W"))
        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0x41))
        drive.closeChannel(2)

        let entry = try XCTUnwrap(drive.findFile("RECOVER"))
        XCTAssertEqual(entry.typeName, "SEQ")
        XCTAssertEqual(drive.readFileData(entry), [0x41])
    }

    func testOpenReplaceWriteChannelCanTruncateExistingFileToZeroBlocks() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "TRUNC,S", data: Array("DATA".utf8)))
        let freeAfterSave = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 2, filename: "@TRUNC,S,W"))
        drive.closeChannel(2)

        let entry = try XCTUnwrap(drive.findFile("TRUNC"))
        XCTAssertEqual(entry.typeName, "SEQ")
        XCTAssertEqual(entry.fileSize, 0)
        XCTAssertEqual(entry.firstTrack, 0)
        XCTAssertEqual(entry.firstSector, 0)
        XCTAssertEqual(drive.readFileData(entry), [])
        XCTAssertEqual(drive.freeBlocks(), freeAfterSave + 1)
    }

    func testOpenWriteChannelAcceptsDrivePrefixedReplaceSyntax() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "LOG,S,W"))
        for byte in Array("OLD".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)
        let freeAfterFirstWrite = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 2, filename: "0:@LOG,S,W"))
        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")
        for byte in Array("N".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)

        XCTAssertEqual(drive.directory.count, 1)
        let entry = try XCTUnwrap(drive.findFile("LOG,S"))
        XCTAssertEqual(entry.typeName, "SEQ")
        XCTAssertEqual(drive.readFileData(entry), Array("N".utf8))
        XCTAssertEqual(drive.freeBlocks(), freeAfterFirstWrite)
        XCTAssertEqual(drive.directory.map(\.filename), ["log"])
    }

    func testOpenAppendChannelRewritesExistingFileWithAppendedData() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "LOG,S,W"))
        for byte in Array("ONE".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)
        let freeAfterFirstWrite = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 2, filename: "LOG,S,A"))
        for byte in Array("TWO".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)

        let entry = try XCTUnwrap(drive.findFile("LOG"))
        XCTAssertEqual(entry.typeName, "SEQ")
        XCTAssertEqual(drive.readFileData(entry), Array("ONETWO".utf8))
        XCTAssertEqual(drive.freeBlocks(), freeAfterFirstWrite)
    }

    func testOpenAppendChannelHonorsRequestedFileType() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeSameNamePRGAndSEQD64()))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "HELLO,S,A"))
        for byte in Array("!".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)

        let prg = try XCTUnwrap(drive.findFile("HELLO,P"))
        let seq = try XCTUnwrap(drive.findFile("HELLO,S"))
        XCTAssertEqual(drive.readFileData(prg), [0xAA, 0xBB])
        XCTAssertEqual(drive.readFileData(seq), [0x10, 0x20, 0x30, 0x21])
    }

    func testOpenAppendChannelReportsExistingFileReadErrorWithoutMutating() throws {
        let sourceDrive = DiskDrive()
        XCTAssertTrue(sourceDrive.mount(makeBlankWritableD64WithErrorTable()))
        XCTAssertTrue(sourceDrive.openFile(channel: 2, filename: "LOG,S,W"))
        for byte in Array("ONE".utf8) {
            XCTAssertTrue(sourceDrive.writeByte(channel: 2, byte: byte))
        }
        sourceDrive.closeChannel(2)
        let existing = try XCTUnwrap(sourceDrive.findFile("LOG,S"))
        let freeAfterSave = sourceDrive.freeBlocks()

        var image = [UInt8](try XCTUnwrap(sourceDrive.exportedD64Image))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        let ordinal = geometry.trackOffsets[Int(existing.firstTrack)] / 256 + Int(existing.firstSector)
        image[errorOffset + ordinal] = 23

        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertFalse(drive.openFile(channel: 2, filename: "LOG,S,A"))

        XCTAssertEqual(drive.currentCommandStatus, "23, READ ERROR,01,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
        let entry = try XCTUnwrap(drive.findFile("LOG,S"))
        XCTAssertEqual(entry.firstTrack, existing.firstTrack)
        XCTAssertEqual(entry.firstSector, existing.firstSector)
        XCTAssertEqual(drive.freeBlocks(), freeAfterSave)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testOpenWriteChannelRejectsWriteProtectedD64() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        drive.setWriteProtected(true)

        XCTAssertFalse(drive.openFile(channel: 2, filename: "LOCKED,S,W"))
        XCTAssertEqual(drive.currentCommandStatus, "26, WRITE PROTECT ON,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testCommandChannelScratchDeletesFileAndReportsStatus() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()
        XCTAssertTrue(drive.savePRG(filename: "DELETE", data: [0x01, 0x08, 0xA9, 0x2A]))
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 1)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:DELETE"))

        XCTAssertNil(drive.findFile("DELETE"))
        XCTAssertEqual(drive.directory.count, 0)
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
        XCTAssertTrue(drive.hasUnsavedChanges)
        XCTAssertTrue(readChannelString(drive, channel: 15).hasPrefix("01, FILES SCRATCHED"))
    }

    func testCommandChannelExecutesBufferedOutputOnClose() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()
        XCTAssertTrue(drive.savePRG(filename: "DELETE", data: [0x01, 0x08, 0xA9, 0x2A]))

        XCTAssertTrue(drive.openFile(channel: 15, filename: ""))
        for byte in Array("S:DELETE\r".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 15, byte: byte))
        }
        drive.closeChannel(15)

        XCTAssertNil(drive.findFile("DELETE"))
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
        XCTAssertTrue(drive.openFile(channel: 15, filename: ""))
        XCTAssertTrue(readChannelString(drive, channel: 15).hasPrefix("01, FILES SCRATCHED"))
    }

    func testCommandChannelScratchWildcardDeletesMatchingFiles() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "ONE", data: [0x01, 0x08, 0x01]))
        XCTAssertTrue(drive.savePRG(filename: "TWO", data: [0x01, 0x08, 0x02]))
        XCTAssertTrue(drive.savePRG(filename: "THREE", data: [0x01, 0x08, 0x03]))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:T*"))

        XCTAssertNotNil(drive.findFile("ONE"))
        XCTAssertNil(drive.findFile("TWO"))
        XCTAssertNil(drive.findFile("THREE"))
        XCTAssertTrue(readChannelString(drive, channel: 15).hasPrefix("02, FILES SCRATCHED"))
    }

    func testCommandChannelScratchHonorsRequestedFileType() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeSameNamePRGAndSEQD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:HELLO,S"))

        XCTAssertNotNil(drive.findFile("HELLO,P"))
        XCTAssertNil(drive.findFile("HELLO,S"))
        XCTAssertTrue(readChannelString(drive, channel: 15).hasPrefix("01, FILES SCRATCHED"))
    }

    func testCommandChannelScratchMissingFileReportsFileNotFound() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:MISSING"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "62, FILE NOT FOUND,00,00\r")
        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: ""))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelScratchReportsD64DirectorySectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 683))
        image[174848 + 358] = 23

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:HELLO"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,18,01\r")
        XCTAssertNotNil(drive.findFile("HELLO"))
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelScratchReportsFileChainReadErrorsWithoutFreeingChain() throws {
        let sourceDrive = DiskDrive()
        XCTAssertTrue(sourceDrive.mount(makeBlankWritableD64WithErrorTable()))
        XCTAssertTrue(sourceDrive.savePRG(filename: "DELETE", data: [0x01, 0x08, 0xA9, 0x2A]))
        let entry = try XCTUnwrap(sourceDrive.findFile("DELETE"))
        let freeAfterSave = sourceDrive.freeBlocks()

        var image = [UInt8](try XCTUnwrap(sourceDrive.exportedD64Image))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        let ordinal = geometry.trackOffsets[Int(entry.firstTrack)] / 256 + Int(entry.firstSector)
        image[errorOffset + ordinal] = 23

        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:DELETE"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,01,00\r")
        let remaining = try XCTUnwrap(drive.findFile("DELETE"))
        XCTAssertEqual(remaining.firstTrack, entry.firstTrack)
        XCTAssertEqual(remaining.firstSector, entry.firstSector)
        XCTAssertEqual(drive.freeBlocks(), freeAfterSave)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelScratchWildcardReadErrorDoesNotPartiallyDeleteMatches() throws {
        let sourceDrive = DiskDrive()
        XCTAssertTrue(sourceDrive.mount(makeBlankWritableD64WithErrorTable()))
        XCTAssertTrue(sourceDrive.savePRG(filename: "TEMP1", data: [0x01, 0x08, 0x11]))
        XCTAssertTrue(sourceDrive.savePRG(filename: "TEMP2", data: [0x01, 0x08, 0x22]))
        let firstEntry = try XCTUnwrap(sourceDrive.findFile("TEMP1"))
        let secondEntry = try XCTUnwrap(sourceDrive.findFile("TEMP2"))
        let freeAfterSave = sourceDrive.freeBlocks()

        var image = [UInt8](try XCTUnwrap(sourceDrive.exportedD64Image))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        let ordinal = geometry.trackOffsets[Int(secondEntry.firstTrack)] / 256 + Int(secondEntry.firstSector)
        image[errorOffset + ordinal] = 23

        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S:TEMP*"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,01,01\r")
        let remainingFirst = try XCTUnwrap(drive.findFile("TEMP1"))
        let remainingSecond = try XCTUnwrap(drive.findFile("TEMP2"))
        XCTAssertEqual(remainingFirst.firstTrack, firstEntry.firstTrack)
        XCTAssertEqual(remainingFirst.firstSector, firstEntry.firstSector)
        XCTAssertEqual(remainingSecond.firstTrack, secondEntry.firstTrack)
        XCTAssertEqual(remainingSecond.firstSector, secondEntry.firstSector)
        XCTAssertEqual(drive.freeBlocks(), freeAfterSave)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelRenameUpdatesDirectoryOnly() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "OLD", data: [0x01, 0x08, 0xA9, 0x2A]))
        let freeAfterSave = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 15, filename: "R:NEW=OLD"))

        XCTAssertNil(drive.findFile("OLD"))
        let entry = try XCTUnwrap(drive.findFile("0:NEW"))
        XCTAssertEqual(drive.readFileData(entry), [0x01, 0x08, 0xA9, 0x2A])
        XCTAssertEqual(drive.freeBlocks(), freeAfterSave)
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
    }

    func testCommandChannelRenameHonorsRequestedSourceFileType() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeSameNamePRGAndSEQD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "R:RENAMED=HELLO,S"))

        XCTAssertNotNil(drive.findFile("HELLO,P"))
        XCTAssertNil(drive.findFile("HELLO,S"))
        let renamed = try XCTUnwrap(drive.findFile("RENAMED,S"))
        XCTAssertEqual(renamed.typeName, "SEQ")
        XCTAssertEqual(drive.readFileData(renamed), [0x10, 0x20, 0x30])
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
    }

    func testCommandChannelAcceptsSpaceSeparatedFileCommandAliases() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "OLD", data: [0x01, 0x08, 0xAA]))
        XCTAssertTrue(drive.savePRG(filename: "COPYME", data: [0x01, 0x08, 0xBB]))
        XCTAssertTrue(drive.savePRG(filename: "DELETE", data: [0x01, 0x08, 0xCC]))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "R NEW=OLD"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertNil(drive.findFile("OLD"))
        XCTAssertNotNil(drive.findFile("NEW"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "COPY CLONE=COPYME"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readFileData(try XCTUnwrap(drive.findFile("CLONE"))), [0x01, 0x08, 0xBB])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "S DELETE"))
        XCTAssertTrue(readChannelString(drive, channel: 15).hasPrefix("01, FILES SCRATCHED"))
        XCTAssertNil(drive.findFile("DELETE"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "NEW FRESH,ID"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.diskName, "fresh")
        XCTAssertEqual(drive.diskID, "id")
        XCTAssertEqual(drive.directory.count, 0)
    }

    func testCommandChannelRenameReportsMissingAndDuplicateErrors() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "ONE", data: [0x01, 0x08, 0x01]))
        XCTAssertTrue(drive.savePRG(filename: "TWO", data: [0x01, 0x08, 0x02]))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "R:THREE=MISSING"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "62, FILE NOT FOUND,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "R:ONE=TWO"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "63, FILE EXISTS,00,00\r")
        XCTAssertNotNil(drive.findFile("ONE"))
        XCTAssertNotNil(drive.findFile("TWO"))
    }

    func testCommandChannelRenameReportsD64DirectorySectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 683))
        image[174848 + 358] = 23

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "R:NEW=HELLO"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,18,01\r")
        XCTAssertNil(drive.findFile("NEW"))
        XCTAssertNotNil(drive.findFile("HELLO"))
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelCopyCreatesIndependentPRGEntry() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "SOURCE", data: [0x01, 0x08, 0xA9, 0x2A]))
        let freeAfterSave = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=SOURCE"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertNotNil(drive.findFile("SOURCE"))
        let copied = try XCTUnwrap(drive.findFile("COPY"))
        XCTAssertEqual(drive.readFileData(copied), [0x01, 0x08, 0xA9, 0x2A])
        XCTAssertEqual(drive.freeBlocks(), freeAfterSave - 1)
        XCTAssertTrue(drive.hasUnsavedChanges)
    }

    func testCommandChannelCopyPreservesSourceFileTypeAndOptions() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "SOURCE,S,W"))
        for byte in Array("SEQ DATA\r".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=SOURCE,S"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let copied = try XCTUnwrap(drive.findFile("COPY"))
        XCTAssertEqual(copied.typeName, "SEQ")
        XCTAssertEqual(drive.readFileData(copied), Array("SEQ DATA\r".utf8))
    }

    func testCommandChannelCopyConcatenatesMultipleSources() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "ONE,S,W"))
        for byte in Array("ONE".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)
        XCTAssertTrue(drive.openFile(channel: 3, filename: "TWO,S,W"))
        for byte in Array("TWO".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 3, byte: byte))
        }
        drive.closeChannel(3)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:JOINED=ONE,TWO"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let copied = try XCTUnwrap(drive.findFile("JOINED"))
        XCTAssertEqual(copied.typeName, "SEQ")
        XCTAssertEqual(drive.readFileData(copied), Array("ONETWO".utf8))
    }

    func testCommandChannelCopyRejectsRELUntilRecordSemanticsAreSupported() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeRELFixtureD64()))
        let initialFreeBlocks = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=INDEX,L"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
        XCTAssertNotNil(drive.findFile("INDEX,L"))
        XCTAssertNil(drive.findFile("COPY"))
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelCopyRejectsLaterRELSourceInMultiSourceCopy() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeRELFixtureD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "ONE,S,W"))
        for byte in Array("ONE".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        drive.closeChannel(2)
        drive.markChangesSaved()
        let freeAfterSeqSave = drive.freeBlocks()
        XCTAssertNotNil(drive.findFile("ONE,S"))
        XCTAssertNotNil(drive.findFile("INDEX,L"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:JOINED=ONE,S,INDEX,L"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
        XCTAssertNil(drive.findFile("JOINED"))
        XCTAssertEqual(drive.freeBlocks(), freeAfterSeqSave)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelCopyClearsD64ErrorBytesForCopiedSectors() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.savePRG(filename: "SOURCE", data: [0x01, 0x08, 0xA9, 0x2A]))

        image = [UInt8](try XCTUnwrap(drive.exportedD64Image))
        image[174848 + 1] = 23
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=SOURCE"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let copied = try XCTUnwrap(drive.findFile("COPY"))
        XCTAssertEqual(Int(copied.firstTrack), 1)
        XCTAssertEqual(Int(copied.firstSector), 1)
        XCTAssertEqual(drive.loadFileData(copied), [0x01, 0x08, 0xA9, 0x2A])
        XCTAssertEqual(drive.readSectorErrorCode(track: 1, sector: 1), 0x01)
    }

    func testCommandChannelCopyAcceptsDrivePrefixesAndReportsErrors() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "ONE", data: [0x01, 0x08, 0x01]))
        XCTAssertTrue(drive.savePRG(filename: "TWO", data: [0x01, 0x08, 0x02]))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C0:THREE=0:ONE"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let copied = try XCTUnwrap(drive.findFile("THREE"))
        XCTAssertEqual(drive.readFileData(copied), [0x01, 0x08, 0x01])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:ONE=TWO"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "63, FILE EXISTS,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "COPY:FOUR=MISSING"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "62, FILE NOT FOUND,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:BROKEN"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
    }

    func testCommandChannelCopyReportsD64SourceSectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 683))
        image[174848] = 23
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=HELLO"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,01,00\r")
        XCTAssertNotNil(drive.findFile("HELLO"))
        XCTAssertNil(drive.findFile("COPY"))
    }

    func testCommandChannelCopyReportsDiskFullWithoutOverwritingStatus() throws {
        let sourceDrive = DiskDrive()
        XCTAssertTrue(sourceDrive.mount(makeBlankWritableD64()))
        XCTAssertTrue(sourceDrive.savePRG(filename: "SOURCE", data: [0x01, 0x08, 0xA9, 0x2A]))

        var image = [UInt8](try XCTUnwrap(sourceDrive.exportedD64Image))
        let bamOffset = DiskDrive.trackOffset[18]
        for track in 1...35 where track != 18 {
            let entryOffset = bamOffset + track * 4
            image[entryOffset] = 0
            image[entryOffset + 1] = 0
            image[entryOffset + 2] = 0
            image[entryOffset + 3] = 0
        }

        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertEqual(drive.freeBlocks(), 0)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=SOURCE"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "72, DISK FULL,00,00\r")
        XCTAssertNotNil(drive.findFile("SOURCE"))
        XCTAssertNil(drive.findFile("COPY"))
    }

    func testCommandChannelCopyRejectsReadOnlyDecodedG64() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mountG64(makeMinimalReadableG64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "C:COPY=SOURCE"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelNewFormatsD64DirectoryAndBAM() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let blankFreeBlocks = drive.freeBlocks()
        XCTAssertTrue(drive.savePRG(filename: "OLD", data: [0x01, 0x08, 0xA9, 0x2A]))
        XCTAssertLessThan(drive.freeBlocks(), blankFreeBlocks)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "N:NEW DISK,ND"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.diskName, "new disk")
        XCTAssertEqual(drive.diskID, "nd")
        XCTAssertEqual(drive.directory.count, 0)
        XCTAssertNil(drive.findFile("OLD"))
        XCTAssertEqual(drive.freeBlocks(), blankFreeBlocks)
        XCTAssertTrue(drive.hasUnsavedChanges)

        XCTAssertTrue(drive.savePRG(filename: "AFTER", data: [0x01, 0x08, 0x60]))
        let entry = try XCTUnwrap(drive.findFile("AFTER"))
        XCTAssertEqual(drive.readFileData(entry), [0x01, 0x08, 0x60])
    }

    func testCommandChannelNewAcceptsDrivePrefixAndDefaultID() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "N0:RENAMED"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.diskName, "renamed")
        XCTAssertEqual(drive.diskID, "sw")
        XCTAssertEqual(drive.directory.count, 0)
    }

    func testCommandChannelNewRejectsSyntaxAndReadOnlyDecodedG64() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "N:,ID"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")

        let g64Drive = DiskDrive()
        XCTAssertTrue(g64Drive.mountG64(makeMinimalReadableG64()))
        XCTAssertTrue(g64Drive.openFile(channel: 15, filename: "N:G64,NO"))
        XCTAssertEqual(readChannelString(g64Drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
        XCTAssertFalse(g64Drive.hasUnsavedChanges)
    }

    func testCommandChannelValidateRebuildsBAMFromDirectoryAndFileChains() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let blankFreeBlocks = drive.freeBlocks()
        XCTAssertTrue(drive.savePRG(filename: "KEEP", data: [0x01, 0x08, 0xA9, 0x2A]))
        let expectedFreeBlocks = drive.freeBlocks()
        XCTAssertEqual(expectedFreeBlocks, blankFreeBlocks - 1)

        var corrupted = try XCTUnwrap(drive.exportedD64Image).map { $0 }
        let trackOneBAM = DiskDrive.trackOffset[18] + 1 * 4
        corrupted[trackOneBAM] = 0
        corrupted[trackOneBAM + 1] = 0
        corrupted[trackOneBAM + 2] = 0
        corrupted[trackOneBAM + 3] = 0

        let remounted = DiskDrive()
        XCTAssertTrue(remounted.mount(Data(corrupted)))
        XCTAssertLessThan(remounted.freeBlocks(), expectedFreeBlocks)

        XCTAssertTrue(remounted.openFile(channel: 15, filename: "V:"))

        XCTAssertEqual(readChannelString(remounted, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(remounted.freeBlocks(), expectedFreeBlocks)
        let entry = try XCTUnwrap(remounted.findFile("KEEP"))
        XCTAssertEqual(remounted.readFileData(entry), [0x01, 0x08, 0xA9, 0x2A])
        XCTAssertTrue(remounted.hasUnsavedChanges)
    }

    func testCommandChannelValidateAcceptsAliasesAndRejectsReadOnlyDecodedG64() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "VALIDATE"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "V0:"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "V0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        let g64Drive = DiskDrive()
        XCTAssertTrue(g64Drive.mountG64(makeMinimalReadableG64()))
        XCTAssertTrue(g64Drive.openFile(channel: 15, filename: "V:"))
        XCTAssertEqual(readChannelString(g64Drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
        XCTAssertFalse(g64Drive.hasUnsavedChanges)
    }

    func testCommandChannelValidateReportsD64BAMSectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 357] = 23

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "V:"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,18,00\r")
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockReadUsesBufferPointerLength() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0x04
        image[sectorOffset + 1] = 0xAD
        image[sectorOffset + 2] = 0xBE
        image[sectorOffset + 3] = 0xEF
        image[sectorOffset + 4] = 0x80
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xAD)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xBE)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xEF)
        let last = drive.readByte(channel: 2)
        XCTAssertEqual(last.byte, 0x80)
        XCTAssertTrue(last.eof)
    }

    func testCommandChannelBlockReadAcceptsBinarySyntaxWithBufferPointerLength() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0x03
        image[sectorOffset + 1] = 0xA1
        image[sectorOffset + 2] = 0xA2
        image[sectorOffset + 3] = 0xA3
        XCTAssertTrue(drive.mount(Data(image)))
        let binaryRead = String(decoding: [0x42, 0x2D, 0x52, 0x3A, 0x02, 0x00, 0x01, 0x02], as: UTF8.self)

        XCTAssertTrue(drive.openFile(channel: 15, filename: binaryRead))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xA1)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xA2)
        let last = drive.readByte(channel: 2)
        XCTAssertEqual(last.byte, 0xA3)
        XCTAssertTrue(last.eof)
    }

    func testCommandChannelUser1CopiesRawSectorIntoTargetChannel() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xDE
        image[sectorOffset + 1] = 0xAD
        image[sectorOffset + 255] = 0x7F
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U1:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xDE)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xAD)
        for _ in 0..<253 {
            _ = drive.readByte(channel: 2)
        }
        let last = drive.readByte(channel: 2)
        XCTAssertEqual(last.byte, 0x7F)
        XCTAssertTrue(last.eof)
    }

    func testCommandChannelBlockReadAcceptsU1AliasAndReportsErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[18]
        image[sectorOffset] = 0x12
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U1 3 0 18 0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 3).byte, 0x12)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U1:15,0,18,0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")

        let binaryCommand = String(decoding: [0x55, 0x31, 0x3A, 0x03, 0x00, 0x12, 0x00], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: binaryCommand))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(Array(readChannelBytes(drive, channel: 3, count: 4).prefix(4)), [18, 1, 65, 0])

        let malformedBinaryCommand = String(decoding: [0x55, 0x31, 0x03, 0x00, 0x12], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: malformedBinaryCommand))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,99,0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "66, ILLEGAL TRACK OR SECTOR,99,00\r")
    }

    func testCommandChannelBlockReadAndExecuteReportDriveNotReadyWithoutDisk() {
        let drive = DiskDrive()

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U1:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
        XCTAssertNil(drive.lastMemoryExecuteAddress)
    }

    func testCommandChannelAcceptsLongBlockCommandAliases() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sourceSectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sourceSectorOffset] = 0x03
        image[sourceSectorOffset + 1] = 0x11
        image[sourceSectorOffset + 2] = 0x22
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "BLOCK-READ:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0x11)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "BLOCK-POINTER:2,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0x22)

        XCTAssertTrue(drive.openFile(channel: 3, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 3, byte: 0xA9))
        XCTAssertTrue(drive.writeByte(channel: 3, byte: 0x42))
        XCTAssertTrue(drive.writeByte(channel: 3, byte: 0x60))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "BLOCK-WRITE:3,0,1,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        let writtenSector = try XCTUnwrap(drive.readSector(track: 1, sector: 3))
        XCTAssertEqual(Array(writtenSector.prefix(3)), [0x03, 0x42, 0x60])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "BLOCK-EXECUTE:4,0,1,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(readChannelBytes(drive, channel: 4, count: 3), [0x03, 0x42, 0x60])

        let freeBlocksAfterWrite = drive.freeBlocks()
        XCTAssertTrue(drive.openFile(channel: 15, filename: "BLOCK-ALLOCATE:0,1,4"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), freeBlocksAfterWrite - 1)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "BLOCK-FREE:0,1,4"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), freeBlocksAfterWrite)
    }

    func testCommandChannelBlockReadReportsD64SectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xDE
        image[sectorOffset + 1] = 0xAD
        image[174848 + 2] = 23
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,01,02\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testCommandChannelFailedBlockReadClearsPreviousTargetChannelData() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0x02
        image[sectorOffset + 1] = 0xDE
        image[sectorOffset + 2] = 0xAD
        image[174848 + 3] = 23
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xDE)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,01,03\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,99,0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "66, ILLEGAL TRACK OR SECTOR,99,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testCommandChannelBlockReadReportsExtendedD64SectorReadErrors() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0, count: 200_704 - image.count))
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 784))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        image[errorOffset + 783] = 23

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,41,15"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,41,15\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testCommandChannelBlockExecuteLoadsTargetBlockBuffer() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xA9
        image[sectorOffset + 1] = 0x42
        image[sectorOffset + 2] = 0x60
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xA9)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0x42)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0x60)
    }

    func testCommandChannelBlockExecuteCopiesSectorToDriveMemoryAndRecordsExecuteAddress() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xA9
        image[sectorOffset + 1] = 0x42
        image[sectorOffset + 2] = 0x60
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.lastMemoryExecuteAddress, 0x0500)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0500,3"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 3), [0xA9, 0x42, 0x60])
    }

    func testCommandChannelBlockExecuteAcceptsBinarySyntaxForDriveMemoryBuffer() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 3 * 256
        image[sectorOffset] = 0x20
        image[sectorOffset + 1] = 0x00
        image[sectorOffset + 2] = 0x04
        XCTAssertTrue(drive.mount(Data(image)))
        let binaryExecute = String(decoding: [0x42, 0x2D, 0x45, 0x3A, 0x03, 0x00, 0x01, 0x03], as: UTF8.self)

        XCTAssertTrue(drive.openFile(channel: 15, filename: binaryExecute))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.lastMemoryExecuteAddress, 0x0600)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0600,3"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 3), [0x20, 0x00, 0x04])
    }

    func testCommandChannelBlockExecuteReportsD64SectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 2] = 23
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,01,02\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testCommandChannelFailedBlockExecuteClearsPreviousExecuteAddress() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        let sectorOffset = DiskDrive.trackOffset[1] + 3 * 256
        image[sectorOffset] = 0xA9
        image[sectorOffset + 1] = 0x42
        image[174848 + 2] = 23
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,1,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.lastMemoryExecuteAddress, 0x0500)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,01,02\r")
        XCTAssertNil(drive.lastMemoryExecuteAddress)
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testCommandChannelBlockPointerSeeksReadBuffer() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xDE
        image[sectorOffset + 1] = 0xAD
        image[sectorOffset + 2] = 0xBE
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xBE)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,256"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
    }

    func testCommandChannelAcceptsBufferPointerAlias() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0x04
        image[sectorOffset + 1] = 0x11
        image[sectorOffset + 2] = 0x22
        image[sectorOffset + 3] = 0x33
        image[sectorOffset + 4] = 0x44
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "BUFFER-POINTER:2,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertEqual(drive.readByte(channel: 2).byte, 0x33)
    }

    func testCommandChannelBlockPointerReportsNoChannelForClosedTarget() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "70, NO CHANNEL,00,00\r")

        let binaryPointer = String(decoding: [0x42, 0x2D, 0x50, 0x3A, 0x02, 0x03], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: binaryPointer))
        XCTAssertEqual(readChannelString(drive, channel: 15), "70, NO CHANNEL,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
    }

    func testCommandChannelBlockWriteCopiesOutputBufferToSectorAndClearsD64Error() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 2] = 23
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAD))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xBE))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(4)), [0xDE, 0xAD, 0xBE, 0x00])
        XCTAssertEqual(drive.readSectorErrorCode(track: 1, sector: 2), 0x01)
        XCTAssertTrue(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockWriteReportsNoChannelForClosedTarget() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xDE
        image[sectorOffset + 1] = 0xAD
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "70, NO CHANNEL,00,00\r")
        XCTAssertEqual(Array(try XCTUnwrap(drive.readSector(track: 1, sector: 2)).prefix(2)), [0xDE, 0xAD])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "70, NO CHANNEL,00,00\r")
        XCTAssertEqual(Array(try XCTUnwrap(drive.readSector(track: 1, sector: 2)).prefix(2)), [0xDE, 0xAD])

        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelRawBlockWriteDoesNotDirtyUnchangedSector() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0xDE
        image[sectorOffset + 1] = 0xAD
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAD))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(3)), [0xDE, 0xAD, 0x00])
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockWriteDoesNotDirtyUnchangedBufferPointerSector() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0x03
        image[sectorOffset + 1] = 0xAD
        image[sectorOffset + 2] = 0xBE
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0x03))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAD))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xBE))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(4)), [0x03, 0xAD, 0xBE, 0x00])
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockWriteStoresBufferPointerInFirstSectorByte() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image.replaceSubrange(sectorOffset..<(sectorOffset + 256), with: [UInt8](repeating: 0x55, count: 256))
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,5"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAD))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(8)), [0x07, 0, 0, 0, 0, 0xDE, 0xAD, 0x55])
        XCTAssertEqual(sector[255], 0x55)
    }

    func testOpenNumberedDirectAccessBufferSupportsBlockCommands() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image.replaceSubrange(sectorOffset..<(sectorOffset + 256), with: [UInt8](repeating: 0x55, count: 256))
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#3"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,5"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xC0))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0x64))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,1,2"))

        let writtenSector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(writtenSector.prefix(8)), [0x07, 0, 0, 0, 0, 0xC0, 0x64, 0x55])

        XCTAssertTrue(drive.openFile(channel: 3, filename: "#0"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:3,0,1,2"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:3,5"))
        XCTAssertEqual(drive.readByte(channel: 3).byte, 0xC0)
        XCTAssertEqual(drive.readByte(channel: 3).byte, 0x64)
    }

    func testOpenDirectAccessBufferClearsPreviousCommandStatus() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertFalse(drive.openFile(channel: 2, filename: "MISSING"))
        XCTAssertEqual(drive.currentCommandStatus, "62, FILE NOT FOUND,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#3"))

        XCTAssertEqual(drive.currentCommandStatus, "00, OK,00,00\r")
    }

    func testOpenDirectAccessBufferRejectsInvalidBufferNames() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAA))
        XCTAssertFalse(drive.openFile(channel: 2, filename: "#5"))
        XCTAssertEqual(drive.currentCommandStatus, "30, SYNTAX ERROR,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#4"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xBB))
        XCTAssertFalse(drive.openFile(channel: 2, filename: "#A"))
        XCTAssertEqual(drive.currentCommandStatus, "30, SYNTAX ERROR,00,00\r")
        XCTAssertTrue(drive.readByte(channel: 2).eof)
    }

    func testCommandChannelBlockWriteAcceptsBinarySyntaxWithBufferPointerSemantics() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image.replaceSubrange(sectorOffset..<(sectorOffset + 256), with: [UInt8](repeating: 0x55, count: 256))
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,5"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAD))
        let binaryWrite = String(decoding: [0x42, 0x2D, 0x57, 0x3A, 0x02, 0x00, 0x01, 0x02], as: UTF8.self)

        XCTAssertTrue(drive.openFile(channel: 15, filename: binaryWrite))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(8)), [0x07, 0, 0, 0, 0, 0xDE, 0xAD, 0x55])
        XCTAssertEqual(sector[255], 0x55)
    }

    func testCommandChannelBlockReadModifyWritePreservesLoadedBufferBytes() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0x06
        image[sectorOffset + 1] = 0xA1
        image[sectorOffset + 2] = 0xA2
        image[sectorOffset + 3] = 0xA3
        image[sectorOffset + 4] = 0xA4
        image[sectorOffset + 5] = 0xA5
        image[sectorOffset + 6] = 0xA6
        image[sectorOffset + 7] = 0x77
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xEE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xEF))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(8)), [0x05, 0xA1, 0xA2, 0xEE, 0xEF, 0xA5, 0xA6, 0x77])
    }

    func testCommandChannelBlockReadAdvancesSharedBufferPointerForWrites() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image[sectorOffset] = 0x06
        image[sectorOffset + 1] = 0xA1
        image[sectorOffset + 2] = 0xA2
        image[sectorOffset + 3] = 0xA3
        image[sectorOffset + 4] = 0xA4
        image[sectorOffset + 5] = 0xA5
        image[sectorOffset + 6] = 0xA6
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-R:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xA1)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0xA2)
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xEE))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(7)), [0x04, 0xA1, 0xA2, 0xEE, 0xA4, 0xA5, 0xA6])
    }

    func testCommandChannelUserReadModifyWritePreservesLoadedRawSectorBytes() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image.replaceSubrange(sectorOffset..<(sectorOffset + 256), with: (0...255).map(UInt8.init))
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U1:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,250"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xEE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xEF))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector[248...255]), [248, 249, 0xEE, 0xEF, 252, 253, 254, 255])
    }

    func testCommandChannelUserReadAdvancesSharedBufferPointerForWrites() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        let sectorOffset = DiskDrive.trackOffset[1] + 2 * 256
        image.replaceSubrange(sectorOffset..<(sectorOffset + 256), with: (0...255).map(UInt8.init))
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U1:2,0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readByte(channel: 2).byte, 0)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 1)
        XCTAssertEqual(drive.readByte(channel: 2).byte, 2)
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xEE))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(6)), [0, 1, 2, 0xEE, 4, 5])
    }

    func testCommandChannelBlockWriteClearsExtendedD64SectorError() throws {
        let drive = DiskDrive()
        var image = [UInt8](makeMinimalD64())
        image.append(contentsOf: [UInt8](repeating: 0, count: 200_704 - image.count))
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 784))
        let geometry = try XCTUnwrap(DiskDrive.d64Geometry(forByteCount: image.count))
        let errorOffset = try XCTUnwrap(geometry.errorInfoOffset)
        image[errorOffset + 783] = 23

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0x4C))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0x00))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xC0))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,41,15"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readSectorErrorCode(track: 41, sector: 15), 0x01)
        let sector = try XCTUnwrap(drive.readSector(track: 41, sector: 15))
        XCTAssertEqual(Array(sector.prefix(3)), [0x4C, 0x00, 0xC0])
        XCTAssertTrue(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockWriteRefreshesDirectoryMetadata() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertEqual(drive.directory.count, 0)

        var directorySector = [UInt8](repeating: 0, count: 256)
        directorySector[0] = 0
        directorySector[1] = 0xFF
        directorySector[2] = 0x82
        directorySector[3] = 1
        directorySector[4] = 0
        let name = Array("MANUAL".utf8)
        for index in 0..<16 {
            directorySector[5 + index] = index < name.count ? name[index] : 0xA0
        }
        directorySector[30] = 1

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        for byte in directorySector {
            XCTAssertTrue(drive.writeByte(channel: 2, byte: byte))
        }
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,18,1"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let entry = try XCTUnwrap(drive.findFile("MANUAL"))
        XCTAssertEqual(entry.firstTrack, 1)
        XCTAssertEqual(entry.firstSector, 0)
        XCTAssertEqual(entry.fileSize, 1)
        XCTAssertEqual(drive.directory.count, 1)
    }

    func testCommandChannelBlockPointerSeeksWriteBuffer() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-P:2,5"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAD))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(8)), [0, 0, 0, 0, 0, 0xDE, 0xAD, 0])
    }

    func testCommandChannelBlockPointerAcceptsBinarySyntax() throws {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        let binaryPointer = String(decoding: [0x42, 0x2D, 0x50, 0x3A, 0x02, 0x05], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: binaryPointer))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xAD))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2:2,0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        let sector = try XCTUnwrap(drive.readSector(track: 1, sector: 2))
        XCTAssertEqual(Array(sector.prefix(8)), [0, 0, 0, 0, 0, 0xDE, 0xAD, 0])

        let malformedPointer = String(decoding: [0x42, 0x2D, 0x50, 0x02], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: malformedPointer))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
    }

    func testCommandChannelBlockWriteAcceptsU2AliasAndReportsErrors() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0x42))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U2 2 0 1 3"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0x99))
        let binaryCommand = String(decoding: [0x55, 0x32, 0x3A, 0x02, 0x00, 0x01, 0x04], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: binaryCommand))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readSector(track: 1, sector: 4)?.first, 0x99)

        let malformedBinaryCommand = String(decoding: [0x55, 0x32, 0x02, 0x00, 0x01], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: malformedBinaryCommand))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-W:2,0,99,0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "66, ILLEGAL TRACK OR SECTOR,99,00\r")

        let g64Drive = DiskDrive()
        XCTAssertTrue(g64Drive.mountG64(makeMinimalReadableG64()))
        XCTAssertTrue(g64Drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(g64Drive.openFile(channel: 15, filename: "B-W:2,0,1,0"))
        XCTAssertEqual(readChannelString(g64Drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
    }

    func testCommandChannelBlockAllocateAndFreeUpdateBAM() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 1)
        XCTAssertTrue(drive.hasUnsavedChanges)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "65, NO BLOCK,01,03\r")
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 1)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-F:0,1,2"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
    }

    func testCommandChannelBlockFreeAlreadyFreeSectorDoesNotDirtyD64() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-F:0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockAllocateAndFreeAcceptBinarySyntax() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()
        let allocate = String(decoding: [0x42, 0x2D, 0x41, 0x3A, 0x00, 0x01, 0x02], as: UTF8.self)
        let free = String(decoding: [0x42, 0x2D, 0x46, 0x3A, 0x00, 0x01, 0x02], as: UTF8.self)

        XCTAssertTrue(drive.openFile(channel: 15, filename: allocate))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks - 1)

        XCTAssertTrue(drive.openFile(channel: 15, filename: free))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)

        let malformedAllocate = String(decoding: [0x42, 0x2D, 0x41, 0x00, 0x01], as: UTF8.self)
        XCTAssertTrue(drive.openFile(channel: 15, filename: malformedAllocate))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
    }

    func testCommandChannelBlockAllocateNextFreeHintStaysWithinStandardBAMTracksOnExtendedD64() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        image.append(contentsOf: [UInt8](repeating: 0, count: 196_608 - image.count))
        XCTAssertTrue(drive.mount(Data(image)))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,35,16"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,35,16"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "65, NO BLOCK,01,00\r")
    }

    func testCommandChannelBlockAllocateReportsErrorsAndReadOnlyMedia() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,99,0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "66, ILLEGAL TRACK OR SECTOR,99,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:1,1,0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")

        let g64Drive = DiskDrive()
        XCTAssertTrue(g64Drive.mountG64(makeMinimalReadableG64()))
        XCTAssertTrue(g64Drive.openFile(channel: 15, filename: "B-A:0,1,0"))
        XCTAssertEqual(readChannelString(g64Drive, channel: 15), "74, DRIVE NOT READY,00,00\r")
        XCTAssertFalse(g64Drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockAllocateRejectsExtendedD64TracksWithoutCorruptingBAMName() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64())
        image.append(contentsOf: [UInt8](repeating: 0, count: 196_608 - image.count))
        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertEqual(drive.diskName, "write test")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,36,0"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "66, ILLEGAL TRACK OR SECTOR,36,00\r")
        XCTAssertEqual(drive.diskName, "write test")
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockAllocateReportsD64BAMSectorReadErrors() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 357] = 23

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "23, READ ERROR,18,00\r")
        XCTAssertFalse(drive.hasUnsavedChanges)
    }

    func testCommandChannelBlockAllocateClearsBAMSectorErrorByte() {
        let drive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 357] = 0x05

        XCTAssertTrue(drive.mount(Data(image)))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-A:0,1,2"))

        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.readSectorErrorCode(track: 18, sector: 0), 0x01)
    }

    func testCommandChannelMemoryReadWriteNumericSyntax() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-W:$0400,3,$DE,$AD,$BE"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0400,3"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 3), [0xDE, 0xAD, 0xBE])
    }

    func testCommandChannelMemoryWriteCountZeroWritesFullPageInNumericSyntax() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let bytes = (0...255).map { "$" + String(format: "%02X", $0) }.joined(separator: ",")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-W:$0500,0,\(bytes)"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0500,0"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 256), Array(0...255).map(UInt8.init))
    }

    func testCommandChannelAcceptsLongMemoryCommandAliases() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "MEMORY-WRITE:$0410,2,$12,$34"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "MEMORY-READ:$0410,2"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 2), [0x12, 0x34])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "MEMORY-EXECUTE:$0410"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertEqual(drive.lastMemoryExecuteAddress, 0x0410)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "MEMORY-READ:$10000,1"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
    }

    func testCommandChannelMemoryReadWriteBinarySyntax() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let writeCommand = String(decoding: [0x4D, 0x2D, 0x57, 0x34, 0x12, 0x03, 0x10, 0x20, 0x30], as: UTF8.self)
        let readCommand = String(decoding: [0x4D, 0x2D, 0x52, 0x34, 0x12, 0x03], as: UTF8.self)

        XCTAssertTrue(drive.openFile(channel: 15, filename: writeCommand))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: readCommand))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 3), [0x10, 0x20, 0x30])
    }

    func testCommandChannelMemoryReadExecutesBufferedOutputOnReturn() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 15, filename: ""))

        for byte in Array("M-W:$0500,2,$CA,$FE\r".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 15, byte: byte))
        }
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        for byte in Array("M-R:$0500,2\r".utf8) {
            XCTAssertTrue(drive.writeByte(channel: 15, byte: byte))
        }
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 2), [0xCA, 0xFE])
    }

    func testCommandChannelInitializeRereadsDirectoryAndReportsErrors() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.savePRG(filename: "KEEP", data: [0x01, 0x08, 0x60]))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "I0:"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertNotNil(drive.findFile("KEEP"))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "I0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertNotNil(drive.findFile("KEEP"))

        let brokenDrive = DiskDrive()
        var image = [UInt8](makeBlankWritableD64WithErrorTable())
        image[174848 + 358] = 23
        XCTAssertTrue(brokenDrive.mount(Data(image)))
        XCTAssertTrue(brokenDrive.openFile(channel: 15, filename: "INITIALIZE"))
        XCTAssertEqual(readChannelString(brokenDrive, channel: 15), "23, READ ERROR,18,01\r")
    }

    func testCommandChannelResetClearsChannelsAndDriveMemory() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        XCTAssertTrue(drive.openFile(channel: 2, filename: "#"))
        XCTAssertTrue(drive.writeByte(channel: 2, byte: 0xDE))
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-W:$0400,1,$7F"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-E:$0400"))
        XCTAssertEqual(drive.lastMemoryExecuteAddress, 0x0400)

        XCTAssertTrue(drive.openFile(channel: 15, filename: "UJ"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertNil(drive.lastMemoryExecuteAddress)

        XCTAssertTrue(drive.readByte(channel: 2).eof)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0400,1"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 1), [0x00])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-W:$0400,1,$7F"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-E:$0400"))
        XCTAssertEqual(drive.lastMemoryExecuteAddress, 0x0400)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "UI+"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertNil(drive.lastMemoryExecuteAddress)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0400,1"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 1), [0x00])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "UI-"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-W:$0400,1,$55"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertTrue(drive.openFile(channel: 15, filename: "B-E:2,0,18,0"))
        XCTAssertNotNil(drive.lastMemoryExecuteAddress)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "U0"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
        XCTAssertNil(drive.lastMemoryExecuteAddress)
        XCTAssertTrue(drive.openFile(channel: 15, filename: "M-R:$0400,1"))
        XCTAssertEqual(readChannelBytes(drive, channel: 15, count: 1), [0x00])

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U0:"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")
    }

    func testCommandChannelAcceptsSupportedDeviceAddressChanges() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U0>9"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U0:>11"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U0>" + String(UnicodeScalar(10)!)))
        XCTAssertEqual(readChannelString(drive, channel: 15), "00, OK,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U0>12"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")

        XCTAssertTrue(drive.openFile(channel: 15, filename: "U0>"))
        XCTAssertEqual(readChannelString(drive, channel: 15), "30, SYNTAX ERROR,00,00\r")
    }

    func testSavePRGTooLargeFailsWithoutConsumingFreeBlocks() {
        let drive = DiskDrive()
        XCTAssertTrue(drive.mount(makeBlankWritableD64()))
        let initialFreeBlocks = drive.freeBlocks()
        let oversizedPayload = [UInt8](repeating: 0xEA, count: 200_000)

        XCTAssertFalse(drive.savePRG(filename: "TOO BIG", data: oversizedPayload))

        XCTAssertEqual(drive.currentCommandStatus, "72, DISK FULL,00,00\r")
        XCTAssertEqual(drive.directory.count, 0)
        XCTAssertEqual(drive.freeBlocks(), initialFreeBlocks)
    }

    private func makeSameNamePRGAndSEQD64() -> Data {
        var image = [UInt8](makeMinimalD64(fileBlocks: 1))

        let prgOffset = DiskDrive.trackOffset[1]
        image[prgOffset + 0] = 0
        image[prgOffset + 1] = 4
        image[prgOffset + 2] = 0xAA
        image[prgOffset + 3] = 0xBB

        let seqOffset = DiskDrive.trackOffset[1] + 256
        image[seqOffset + 0] = 0
        image[seqOffset + 1] = 5
        image[seqOffset + 2] = 0x10
        image[seqOffset + 3] = 0x20
        image[seqOffset + 4] = 0x30

        let dirOffset = DiskDrive.trackOffset[18] + 256
        let secondEntry = dirOffset + 32
        image[secondEntry + 2] = 0x81
        image[secondEntry + 3] = 1
        image[secondEntry + 4] = 1
        let name = Array("HELLO".utf8)
        for index in 0..<16 {
            image[secondEntry + 5 + index] = index < name.count ? name[index] : 0xA0
        }
        image[secondEntry + 30] = 1

        return Data(image)
    }

    private func makeRELFixtureD64() -> Data {
        var image = [UInt8](makeBlankWritableD64())

        let relOffset = DiskDrive.trackOffset[1]
        image[relOffset + 0] = 0
        image[relOffset + 1] = 5
        image[relOffset + 2] = 0x52
        image[relOffset + 3] = 0x45
        image[relOffset + 4] = 0x4C

        let bamOffset = DiskDrive.trackOffset[18]
        image[bamOffset + 1] &= ~UInt8(0x01)
        image[bamOffset + 4] = image[bamOffset + 4] &- 1

        let dirOffset = DiskDrive.trackOffset[18] + 256
        image[dirOffset + 2] = 0x84
        image[dirOffset + 3] = 1
        image[dirOffset + 4] = 0
        let name = Array("INDEX".utf8)
        for index in 0..<16 {
            image[dirOffset + 5 + index] = index < name.count ? name[index] : 0xA0
        }
        image[dirOffset + 30] = 1

        return Data(image)
    }

    private func makeBlankWritableD64() -> Data {
        let totalBytes = 174848
        var image = [UInt8](repeating: 0, count: totalBytes)
        let bamOffset = DiskDrive.trackOffset[18]

        image[bamOffset + 0] = 18
        image[bamOffset + 1] = 1
        image[bamOffset + 2] = 0x41

        for track in 1...35 {
            let entryOffset = bamOffset + track * 4
            let sectors = DiskDrive.sectorsPerTrack[track]
            var bitmap = [UInt8](repeating: 0, count: 3)
            for sector in 0..<sectors {
                bitmap[sector / 8] |= 1 << UInt8(sector % 8)
            }

            if track == 18 {
                bitmap[0] &= ~UInt8(0x03)
                image[entryOffset] = UInt8(sectors - 2)
            } else {
                image[entryOffset] = UInt8(sectors)
            }

            image[entryOffset + 1] = bitmap[0]
            image[entryOffset + 2] = bitmap[1]
            image[entryOffset + 3] = bitmap[2]
        }

        let name = Array("WRITE TEST".utf8)
        for i in 0..<16 {
            image[bamOffset + 0x90 + i] = i < name.count ? name[i] : 0xA0
        }
        image[bamOffset + 0xA2] = 0x53
        image[bamOffset + 0xA3] = 0x57
        image[bamOffset + 0xA5] = 0x32
        image[bamOffset + 0xA6] = 0x41

        let dirOffset = DiskDrive.trackOffset[18] + 256
        image[dirOffset + 0] = 0
        image[dirOffset + 1] = 0xFF

        return Data(image)
    }

    private func makeBlankWritableD64WithErrorTable() -> Data {
        var image = [UInt8](makeBlankWritableD64())
        image.append(contentsOf: [UInt8](repeating: 0x01, count: 683))
        return Data(image)
    }

    private func readChannelString(_ drive: DiskDrive, channel: Int) -> String {
        var bytes: [UInt8] = []
        while true {
            let result = drive.readByte(channel: channel)
            bytes.append(result.byte)
            if result.eof { break }
        }
        return String(decoding: bytes, as: UTF8.self)
    }

    private func readChannelBytes(_ drive: DiskDrive, channel: Int, count: Int) -> [UInt8] {
        (0..<count).map { _ in drive.readByte(channel: channel).byte }
    }

    private func makeMinimalReadableG64() -> Data {
        var sector = [UInt8](repeating: 0, count: 256)
        sector[0] = 18
        sector[1] = 1
        sector[2] = 0x41
        return Data(buildG64(trackSectors: [18: [(0, sector)]]))
    }

    private func buildG64(trackSectors: [Int: [(Int, [UInt8])]]) -> [UInt8] {
        let numTracks = 84
        var g64 = [UInt8]()

        g64.append(contentsOf: [0x47, 0x43, 0x52, 0x2D, 0x31, 0x35, 0x34, 0x31])
        g64.append(0x00)
        g64.append(UInt8(numTracks))
        g64.append(0x00)
        g64.append(0x1E)

        let offsetTablePosition = g64.count
        g64.append(contentsOf: [UInt8](repeating: 0, count: numTracks * 4))
        g64.append(contentsOf: [UInt8](repeating: 0, count: numTracks * 4))

        for track in trackSectors.keys.sorted() {
            let halfTrack = (track - 1) * 2
            guard let sectors = trackSectors[track] else { continue }
            let trackDataOffset = g64.count
            let trackGCR = buildGCRTrack(track: track, sectors: sectors)
            g64.append(UInt8(trackGCR.count & 0xFF))
            g64.append(UInt8(trackGCR.count >> 8))
            g64.append(contentsOf: trackGCR)

            let offsetPosition = offsetTablePosition + halfTrack * 4
            g64[offsetPosition] = UInt8(trackDataOffset & 0xFF)
            g64[offsetPosition + 1] = UInt8((trackDataOffset >> 8) & 0xFF)
            g64[offsetPosition + 2] = UInt8((trackDataOffset >> 16) & 0xFF)
            g64[offsetPosition + 3] = UInt8((trackDataOffset >> 24) & 0xFF)
        }

        return g64
    }

    private func buildGCRTrack(track: Int, sectors: [(Int, [UInt8])]) -> [UInt8] {
        var gcr = [UInt8]()

        for (sectorNumber, sectorData) in sectors {
            gcr.append(contentsOf: [UInt8](repeating: 0xFF, count: 5))
            let id1: UInt8 = 0x41
            let id2: UInt8 = 0x42
            let headerChecksum = UInt8(sectorNumber) ^ UInt8(track) ^ id2 ^ id1
            let header: [UInt8] = [0x08, headerChecksum, UInt8(sectorNumber), UInt8(track), id2, id1, 0x0F, 0x0F]
            gcr.append(contentsOf: encodeGCRBytes(header))
            gcr.append(contentsOf: [UInt8](repeating: 0x55, count: 9))
            gcr.append(contentsOf: [UInt8](repeating: 0xFF, count: 5))

            var dataBlock = [UInt8]()
            dataBlock.append(0x07)
            dataBlock.append(contentsOf: sectorData)
            dataBlock.append(sectorData.reduce(0, ^))
            dataBlock.append(0x00)
            dataBlock.append(0x00)
            gcr.append(contentsOf: encodeGCRBytes(dataBlock))
            gcr.append(contentsOf: [UInt8](repeating: 0x55, count: 8))
        }

        return gcr
    }

    private func encodeGCRBytes(_ data: [UInt8]) -> [UInt8] {
        var padded = data
        while padded.count % 4 != 0 { padded.append(0) }
        var result = [UInt8]()
        for index in stride(from: 0, to: padded.count, by: 4) {
            result.append(contentsOf: encodeGCR(Array(padded[index..<index + 4])))
        }
        return result
    }

    private func encodeGCR(_ bytes: [UInt8]) -> [UInt8] {
        let encode: [UInt8] = [0x0A, 0x0B, 0x12, 0x13, 0x0E, 0x0F, 0x16, 0x17, 0x09, 0x19, 0x1A, 0x1B, 0x0D, 0x1D, 0x1E, 0x15]
        let g0 = UInt64(encode[Int(bytes[0] >> 4)])
        let g1 = UInt64(encode[Int(bytes[0] & 0x0F)])
        let g2 = UInt64(encode[Int(bytes[1] >> 4)])
        let g3 = UInt64(encode[Int(bytes[1] & 0x0F)])
        let g4 = UInt64(encode[Int(bytes[2] >> 4)])
        let g5 = UInt64(encode[Int(bytes[2] & 0x0F)])
        let g6 = UInt64(encode[Int(bytes[3] >> 4)])
        let g7 = UInt64(encode[Int(bytes[3] & 0x0F)])
        let bits = (g0 << 35) | (g1 << 30) | (g2 << 25) | (g3 << 20) | (g4 << 15) | (g5 << 10) | (g6 << 5) | g7

        return [
            UInt8((bits >> 32) & 0xFF),
            UInt8((bits >> 24) & 0xFF),
            UInt8((bits >> 16) & 0xFF),
            UInt8((bits >> 8) & 0xFF),
            UInt8(bits & 0xFF),
        ]
    }
}
